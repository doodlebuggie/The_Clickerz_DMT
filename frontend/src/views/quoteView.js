...
export function renderQuoteView(container, user, onQuote) {
  ...
  `;
    ...
    const errDiv = container.querySelector('#quote-error');
    const searchInput = container.querySelector('#receiver-search');
    const searchBtn = container.querySelector('#search-btn');
    const resultsList = container.querySelector('#search-results');
    const receiverInput = container.querySelector('#receiver-wallet');
    const receiverDisplay = container.querySelector('#receiver-display');
    const amountInput = container.querySelector('#amount');
    const amountCurrency = container.querySelector('#amount-currency');
    let senderWalletInfo = null;
    let recipientWalletInfo = null;
    let currentPaymentType = 'FIXED_SEND';
    ...
    function renderRecipientCard(result, currency) {
    ...
    async function selectUser(result) {
        ...
        if (result.walletAddress) {
            ...
            const tag = receiverDisplay.querySelector('#recipient-currency-tag');
            if (tag)
                tag.textContent = recipientWalletInfo?.assetCode ?? '?';
            if (currentPaymentType === 'FIXED_RECEIVE')
    ...
    form.querySelectorAll('input[name="paymentType"]').forEach(radio => {
        radio.addEventListener('change', () => {
            currentPaymentType = radio.value;
            amountInput.value = '';
            updateAmountCurrency();
        });
    });
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const rawAmount = parseFloat(data.get('amount'));
