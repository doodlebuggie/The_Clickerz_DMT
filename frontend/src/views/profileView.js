...
export async function renderProfileView(container) {
  ...
  `;
    // Avatar preview
    const avatarInput = container.querySelector('#p-avatar');
    const avatarPreview = container.querySelector('#avatar-preview');
    const avatarPH = container.querySelector('#avatar-placeholder');
    ...
    const errDiv = container.querySelector('#profile-error');
    const successDiv = container.querySelector('#profile-success');
    form.addEventListener('submit', async (e) => {
        ...
        try {
            const data = {
                displayName: (form.querySelector('#p-name').value).trim() || undefined,
                email: (form.querySelector('#p-email').value).trim() || undefined,
                walletAddress: (form.querySelector('#p-wallet').value).trim() || '',
                password: form.querySelector('#p-password').value || undefined,
            };
            const file = avatarInput.files?.[0];
            if (file) {
            ...
            container.querySelector('#profile-name-display').textContent = updated.displayName;
